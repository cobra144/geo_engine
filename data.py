        ...
        if Normals:
            if i==0:
                ...
                if i % sampleDensity[0] != sampleDensity[0]-1 and i <  (sampleDensity[1]-1)*sampleDensity[0]:
                    ...
                    if i % sampleDensity[0] == sampleDensity[0]-1 and i < (sampleDensity[1] - 1) * sampleDensity[0]:#prawa kolumna
                        ...
                        if i != (SampleDensity[0]-1)*SampleDensity[1]+SampleDensity[0]-1:
                            ...
                            w1 = [xData[i - SampleDensity[0] - 1], yData[i - SampleDensity[0] - 1], zData[i - SampleDensity[0] - 1]]
                            w2 = [xData[i], yData[i], zData[i]]
...
vertexData = createVertexDataOpenGL(borders, sampleDensity, heights,ColorPattern=demColors, HeightRange=minmaxT(heights), Normals=True, TexCoord = texCoord)
for i in range(9):
