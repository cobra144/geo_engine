...
//uniform vec4 ColorToSphere;  color which has to be send to sphere.frag
...
void main()
{
    ...
    vec3 NewFrontColor = FrontColor;
    //vec3 FrontColor = vec3(1.0, 0.5, 0.7);
    for(int i=0; i<nSamples; i++) {
        ...
        NewFrontColor += Attenuate * (Depth * ScaledLength);
        SamplePoint += SampleRay;
    }

    // Finally, scale the Mie and Rayleigh colors and set up the varying variables for the pixel shader
    vec4 secondaryColor = vec4(NewFrontColor * KmESun, 1.0);
    vec4 primaryColor = vec4(NewFrontColor * (InvWavelength * KrESun), 1.0);
	...
	if (dotLSL > cos(spotLightAngle))
	    //ColorToSphere = (vec4(d*diffuseColor*dotLN,1.0)+vec4(s*specularColor*dotRV,1.0))*dotLSL;
    ...
    //outputColor = texel;
    //outputColor = ((vec4(d*diffuseColor*dotLN,1.0)+vec4(s*specularColor*dotRV,1.0))*dotLSL+texel+
    outputColor = (RayleighPhase * primaryColor + MiePhase * secondaryColor)*lightForce;
